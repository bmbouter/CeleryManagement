...
class TaskStats(object):
    ...
    @property
    def calculate_throughputs(self, timerange, intervale=1):
        ...
        stop = timerange[1]
        if taskname:
            states_in_range = self.qs.filter(name=taskname, state='SUCCESS', 
                                                       tstamp__range=(start, stop))
        else:
            states_in_range = self.qs.objects.filter(state='SUCCESS', 
                                                       tstamp__range=(start, stop))
    ...
    def _calculate_runtimes_query(runtime_range=None, search_range=(None,None)):
        #import pdb; pdb.set_trace()
        
        if runtime_range and runtime_range!=(None,None):
            if taskname:
                self.qs = self.qs.filter(state='SUCCESS', name=taskname, runtime__range=runtime_range)
            else:
                self.qs = self.qs.filter(state='SUCCESS', runtime__range=runtime_range)
        elif taskname:
        ...
        if search_range[0] and search_range[1]:
            self.qs = self.qs.filter(tstamp__range=search_range)
        elif search_range[0]:
            self.qs = self.qs.filter(tstamp__gte=search_range[0])
        elif search_range[1]:
            self.qs = self.qs.filter(tstamp__lte=search_range[1])
            
        return self.qs
        ##return qs.values_list('runtime', flat=True).order_by('runtime')
        
        
    def calculate_auto_runtimes(search_range=(None,None), bin_count=None):
        # Get the 
        runtimeq = _calculate_runtimes_query(search_range)
    ...
    def calculate_explicit_runtimes(search_range=(None,None), 
        ...
        runtimeq = _calculate_runtimes_query(runtime_range, 
                                              search_range )
        runtimes = runtimeq.values_list('runtime', flat=True).order_by('runtime')
        bins = _calculate_runtimes_fillbins(runtimes, runtime_min, bin_size, bin_count)
        return bins
